...
As I was driving to work today, I spoke with my girlfriend on the phone. Somehow the subject came up of whether or not we would have a white Christmas this year. Being that Christmas is only a few days out and the fact that I love anything relating to the environment and data. I figured I would get to the bottom of this question and analyze some historical data to see what the past 10 years have shown. I have read some articles on this topic but figured it would be a fun project to do in R. So in this post I will show you how you can pull data into R and create a cool plot with the ```ggplot2``` package. Since I live in New Jersey, I will analyze data from New Jersey weather stations.

## Acquiring the data

There are numerous ways to access weather data online but for my learning purposes and hopes that this post will be helpful to someone else, I am going to use the ```rnoaa``` package. The [rnoaa](https://docs.ropensci.org/rnoaa/index.html){target="_blank"} package is an R interface to many of NOAA's data sources. It is a package maintained by [rOpenSci](https://ropensci.org/){target="_blank"}. In short, the ```rnoaa``` package makes it extremely easy to access data from NOAA's API. One of the best parts about this package is it brings the data into R in an easy-to-use format. Being that it would take a great deal of time to understand how to make requests to NOAA's API, this package takes away most of the leg work. To start, you need to request a key to gain access to NOAA's API. You can do this by going [here](https://www.ncdc.noaa.gov/cdo-web/token){target="_blank"} and providing your email. Within seconds you should receive an email with your key. 

Now that we have our key, we can access any of the data sources in ```rnoaa```. You can see the full list of data sets by going to [rnoaa's website](https://docs.ropensci.org/rnoaa/index.html){target="_blank"}. For this blog post, I am going to access data from the Global Historical Climate Network (GHCN) dataset. This dataset includes daily land surface observations from weather stations around the world. Since I want snowfall data for the past 10 years, this is a great place to get it. The function in ```rnoaa``` that can do such a query is ```meteo_pull_monitors```. This function takes a vector of one or more weather station IDs from the GHCN dataset and joins them together in a single data frame. Before we can do this, however, we need to figure out which stations we want data from. To do this let's use the ```ncdc_stations``` function and put in the FIPS code for NJ (Middlesex County).
```
